...
namespace BisDll.Stream
{
    ...
    public class BinaryReaderEx : BinaryReader
    {
        ...
        public struct CoverageItem
        {
            public long Start;
            public long End;
            public string Label;
            public void Deconstruct(out long start, out long end, out string label)
            {
                start = Start;
                end = End;
                label = Label;
        ...
        public List<CoverageItem> Coverage { get; set; } = new List<CoverageItem>();
...
namespace BisDll.Model
{
    public static class Conversion
    {
        ...
        private static void ReconstructProxies(LOD src, out Dictionary<string, List<PointWeight>> points, out Dictionary<string, List<int>> faces)
        {
            ...
            for (int i = 0; i < src.Faces.Length; i++)
            {
                ...
                for (int j = 0; j < src.RawProxies.Length; j++)
                {
                    Proxy p = src.RawProxies[j];
                    Vector3P position = p.transformation.Position;
                    Vector3P up = p.transformation.Orientation.Up;
                    Vector3P dir = p.transformation.Orientation.Dir;
                    if (vector3P.Equals(position) && vector3P2.Equals(dir) && vector3P3.Equals(up))
                    {
                        string name = src.NamedSelections[p.namedSelectionIndex].Name;
    ...
    public abstract class P3D_LOD
    {
        ...
        public abstract string[] Selections { get; }

        public abstract string[] Proxies { get; }
    }
...
namespace BisDll.Model.ODOL
{
    ...
    public class LOD : P3D_LOD, IComparable<LOD>
    {
        ...
        public override string[] Selections => namedSelections.Select((NamedSelection ns) => ns.Name).ToArray();

        public override string[] Proxies => proxies.Select((Proxy p) => namedSelections[p.namedSelectionIndex].Name).ToArray();

        public Proxy[] RawProxies => proxies;
...
namespace BisDll.Model.MLOD
{
    ...
    public class MLOD_LOD : P3D_LOD
    {
        ...
        public override string[] Selections => new string[0];

        public override string[] Proxies => new string[0];
